...
function saveAdminSettings(){ localStorage.setItem(SETTINGS_KEY, JSON.stringify(adminSettings)); }
...
function updateTimeLimitUI() {
  ...
  const expired = document.getElementById("timeLimitExpired");
  const anyInput = document.querySelectorAll("#studentForm input, #studentForm select, #studentForm button");
  ...
  if(info) info.innerHTML = `Booking is open.`;
  if(expired) expired.style.display = "none";
  anyInput.forEach(i => i.disabled = false);
}
...
async function showMessList(applyFilter = true) {
  ...
  filtered.forEach(r=>{
    ...
    tr.innerHTML = `<td>${r.roll}</td><td>${r.name}</td><td>${r.hostel}</td><td>${r.room}</td><td>${r.mobile}</td><td>${bookingDate}</td><td>${dayLabel}</td><td>${r.time}</td><td>${foodTypeLabel}</td><td>${r.menuItem}</td>`;
    tbody.append(tr);
  });
 }
...
async function openAdminView(){
  console.log("openAdminView called");
  const pass = window.prompt("Enter admin password to view mess list:");
  if(pass === null) return;
  if(pass !== ADMIN_PASSWORD){ showError(error1, "Invalid admin password."); return; }
...
window.addEventListener("DOMContentLoaded", ()=> {
  ...
  if(mondayToggle){
    mondayToggle.addEventListener("change", ()=>{
  ...
  if(saturdayToggle){
    saturdayToggle.addEventListener("change", ()=>{
  ...
  if(updateWindowButton){
    updateWindowButton.addEventListener("click", (e)=>{
      e.preventDefault();
